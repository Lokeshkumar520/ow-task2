...
async function deleteDevice(body) {
    ...
    if (isDevice) {
        ...
        let response = { status: 'Success', message: isDevice.deviceName + ' Device deleted Successfully' };
        return response;
    }
    let response = { status: 'Failed', message: 'Device is not exist/already removed' };
...
async function getAllDevices() {
    ...
    if (isDevice) {
        io.emit('Device_List', isDevice);
        let response = { status: 'Success', message: 'All Device Listed Successfully', deviceList: isDevice };
        return response;
    }
    let response = { status: 'Failed', message: 'Devices are not exist' }
    return response;
}
...
async function _cleanDump() {
    try {
        ...
        console.log("Cleaned Database, and Test the task fresh Data");
    } catch (error) {
        console.log(error);
    }
}