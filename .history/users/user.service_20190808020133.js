...
async function authenticate({ username, password }) {
    ...
    if (user && bcrypt.compareSycanc(password, user.hash)) {
        ...
        try {            
            return { status: "Success", message: "Successfully logged in ", token: token, profile:userWithoutHash };
        } catch (error) {
            console.error(error);
            return { status: "Failed", message: "Error Please try again!" };
        }
    }
    const response = { status: "Failed", message: "Username or Password is wr! Please try agn" };
    return response
}
